...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    public class FastZip
    {
            ...
            string directoryFilter)
        {
            ...
            {
#if !NETCF_1_0
                if (Password != null) outputStream_.Password = Password;
#endif

                outputStream_.UseZip64 = UseZip64;
                var scanner = new FileSystemScanner(fileFilter, directoryFilter);
                scanner.ProcessFile += ProcessFile;
                if (CreateEmptyDirectories) scanner.ProcessDirectory += ProcessDirectory;

                if (events_ != null)
                {
                    if (events_.FileFailure != null) scanner.FileFailure += events_.FileFailure;

                    if (events_.DirectoryFailure != null) scanner.DirectoryFailure += events_.DirectoryFailure;
                }
            ...
            bool isStreamOwner)
        {
            ...
            {
#if !NETCF_1_0
                if (Password != null) zipFile_.Password = Password;
#endif
                ...
                while (continueRunning_ && enumerator.MoveNext())
                {
                    ...
                    if (entry.IsFile)
                    {
                        ...
                        if (directoryFilter_.IsMatch(Path.GetDirectoryName(entry.Name)) &&
                            fileFilter_.IsMatch(entry.Name)) ExtractEntry(entry);
                    }
                    else if (entry.IsDirectory)
                    {
                        if (directoryFilter_.IsMatch(entry.Name) && CreateEmptyDirectories) ExtractEntry(entry);
                    }
                }
            }
        }
        ...
        private void ProcessDirectory(object sender, DirectoryEventArgs e)
        {
            ...
            {
                if (events_ != null) events_.OnProcessDirectory(e.Name, e.HasMatchingFiles);
        ...
        {
            if (events_ != null && events_.ProcessFile != null) events_.ProcessFile(sender, e);
        ...
        private void AddFileContents(string name, Stream stream)
        {
            ...
            if (buffer_ == null) buffer_ = new byte[4096];
            ...
            if (events_ != null) continueRunning_ = events_.OnCompletedFile(name);
        }
        ...
        private void ExtractFileEntry(ZipEntry entry, string targetName)
        {
            ...
            if (proceed)
            {
                if (events_ != null) continueRunning_ = events_.OnProcessFile(entry.Name);
                ...
                if (continueRunning_)
                    try
                    {
                        using (var outputStream = File.Create(targetName))
                        {
                            ...
                            if (events_ != null) continueRunning_ = events_.OnCompletedFile(entry.Name);
                        }

#if !NETCF_1_0 && !NETCF_2_0
                        if (RestoreDateTimeOnExtract) File.SetLastWriteTime(targetName, entry.DateTime);
        ...
        private void ExtractEntry(ZipEntry entry)
        {
            ...
            if (doExtraction)
            {
                ...
                else if (entry.IsDirectory) targetName = extractNameTransform_.TransformDirectory(targetName);
            ...
            if (doExtraction && entry.IsFile) ExtractFileEntry(entry, targetName);
        }
