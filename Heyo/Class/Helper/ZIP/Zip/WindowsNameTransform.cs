...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    public class WindowsNameTransform : INameTransform
    {
        ...
        {
            if (baseDirectory == null) throw new ArgumentNullException("baseDirectory", "Directory name is invalid");
        ...
        public string TransformFile(string name)
        {
            if (name != null)
            {
                ...
                if (_baseDirectory != null) name = Path.Combine(_baseDirectory, name);
            }
            else
            {
                name = string.Empty;
        ...
        public static string MakeValidName(string name, char replacement)
        {
            ...
            var index = name.IndexOf(string.Format("{0}{0}", Path.DirectorySeparatorChar));
            while (index >= 0)
            {
            ...
            if (name.Length > MaxPath) throw new PathTooLongException();
