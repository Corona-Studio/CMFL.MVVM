...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    public class ZipFile : IEnumerable, IDisposable
    {
        ...
        public ZipFile(FileStream file)
        {
            ...
            if (!file.CanSeek) throw new ArgumentException("Stream is not seekable", "file");
        ...
        public ZipFile(Stream stream)
        {
            ...
            if (!stream.CanSeek) throw new ArgumentException("Stream is not seekable", "stream");
        ...
        private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
        {
            lock (baseStream_)
            {
                    ...
                    if (entry.IsFile)
                    {
                        ...
                        if (extractVersion > ZipConstants.VersionMadeBy
                            ...
                            throw new ZipException(string.Format(
                                "Version required to extract this entry not supported ({0})", extractVersion));

                        if ((localFlags & (int) (GeneralBitFlags.Patched | GeneralBitFlags.StrongEncryption |
                ...
                if (testHeader)
                {
                    ...
                    if ((localFlags & (int) (GeneralBitFlags.ReservedPKware4 | GeneralBitFlags.ReservedPkware14 |
                                             GeneralBitFlags.ReservedPkware15)) !=
                        0) throw new ZipException("Reserved bit flags cannot be set.");

                    // Encryption requires extract version >= 20
                    if ((localFlags & (int) GeneralBitFlags.Encrypted) != 0 && extractVersion < 20)
                        throw new ZipException(string.Format(
                            "Version required to extract this entry is too low for encryption ({0})", extractVersion));
...
//		}
        ...
        {
            if (archiveStorage == null) throw new ArgumentNullException("archiveStorage");
 ...
 */
                    ...
                    if (update != null)
                        switch (update.Command)
                        {
                            ...
                            case UpdateCommand.Add:
                                if (!IsNewArchive && directUpdate) workFile.baseStream_.Position = destinationPosition;

                                AddEntry(workFile, update);

                                if (directUpdate) destinationPosition = workFile.baseStream_.Position;
                                break;
                        }
        ...
        {
            if (updates_ == null) throw new InvalidOperationException("BeginUpdate has not been called");
        }
        ...
        private class ZipUpdate
        {
            ...
            public ZipEntry OutEntry
            {
                get
                {
                    if (outEntry_ == null) outEntry_ = (ZipEntry) Entry.Clone();
            ...
            {
                Stream result = null;
                if (dataSource_ != null) result = dataSource_.GetSource();
        ...
        private void ReadEntries()
        {
            ...
            if (baseStream_.CanSeek == false) throw new ZipException("ZipFile stream must be seekable");
                ...
                offsetOfCentralDir == 0xffffffff)
            {
                ...
                if (sig64 != ZipConstants.Zip64CentralFileHeaderSignature)
                    throw new ZipException(
                        string.Format("Invalid Zip64 Central directory signature at {0:X}", offset64));
        ...
        private class ZipString
        {
            ...
            {
                if (comment_ == null) comment_ = ZipConstants.ConvertToString(rawComment_);
            }

            private void MakeBytesAvailable()
            {
                if (rawComment_ == null) rawComment_ = ZipConstants.ConvertToArray(comment_);
            }
        ...
        private class PartialInputStream : Stream
        {
            ...
            {
                if (readPos_ >= end_) // -1 is the correct value at end of stream.
                    return -1;
    ...
    public class DiskArchiveStorage : BaseArchiveStorage
    {
        ...
        {
            if (temporaryStream_ == null) throw new ZipException("No temporary stream has been created");
        ...
        {
            if (temporaryStream_ != null) temporaryStream_.Close();
        }
