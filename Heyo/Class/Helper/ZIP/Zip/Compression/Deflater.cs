    ...
    public class Deflater
    {
        ...
        public void SetLevel(int level)
        {
            if (level == DEFAULT_COMPRESSION)
                level = 6;
            else if (level < NO_COMPRESSION || level > BEST_COMPRESSION) throw new ArgumentOutOfRangeException("level");
        ...
        public int Deflate(byte[] output, int offset, int length)
        {
            ...
            if (state < BUSY_STATE)
            {
                ...
                if ((state & IS_SETDICT) != 0) // Dictionary was set
            ...
            for (;;)
            {
                ...
                {
                    if (state == BUSY_STATE) // We need more input now
