...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
    ...
    public class DeflaterHuffman
    {
        ...
        public void SendAllTrees(int blTreeCodes)
        {
            ...
            for (var rank = 0; rank < blTreeCodes; rank++) pending.WriteBits(blTree.length[BL_ORDER[rank]], 3);
        ...
        public void FlushBlock(byte[] stored, int storedOffset, int storedLength, bool lastBlock)
        {
            ...
            for (var i = 0; i < LITERAL_NUM; i++) static_len += literalTree.freqs[i] * staticLLength[i];
            for (var i = 0; i < DIST_NUM; i++) static_len += distTree.freqs[i] * staticDLength[i];
            if (opt_len >= static_len) // Force static trees
                opt_len = static_len;
        ...
        private class Tree
        {
            ...
            public void Reset()
            {
                for (var i = 0; i < freqs.Length; i++) freqs[i] = 0;
                codes = null;
                length = null;
            }
            ...
            public void CheckEmpty()
            {
                ...
                for (var i = 0; i < freqs.Length; i++)
                    if (freqs[i] != 0) //Console.WriteLine("freqs[" + i + "] == " + freqs[i]);
                        empty = false;

                if (!empty) throw new SharpZipBaseException("!Empty");
            }
            ...
            public void BuildTree()
            {
                ...
                */
                ...
                if (heap[0] != childs.Length / 2 - 1) throw new SharpZipBaseException("Heap invariant violated");
            ...
            {
                var len = 0;
                for (var i = 0; i < freqs.Length; i++) len += freqs[i] * length[i];
                return len;
            }
            ...
            private void BuildLength(int[] childs)
            {
                ...
                for (var i = 0; i < maxLength; i++) bl_counts[i] = 0;
