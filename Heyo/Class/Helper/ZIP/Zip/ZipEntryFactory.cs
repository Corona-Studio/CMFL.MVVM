...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    public class ZipEntryFactory : IEntryFactory
    {
        ...
        public DateTime FixedDateTime
        {
            ...
            set
            {
                if (value.Year < 1970) throw new ArgumentException("Value is too old to be valid", "value");
        ...
        public ZipEntry MakeFileEntry(string fileName, string entryName, bool useFileSystem)
        {
            var result =
                new ZipEntry(
                    nameTransform_.TransformFile(entryName != null && entryName.Length > 0 ? entryName : fileName));
            result.IsUnicodeText = IsUnicodeText;

            var externalAttributes = 0;
            var useAttributes = SetAttributes != 0;
            ...
            {
                if (Setting == TimeSetting.Fixed) result.DateTime = fixedDateTime_;
            }
